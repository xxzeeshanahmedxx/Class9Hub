...
- **Tag:** A keyword enclosed by angle brackets (like `<tag>`) that tells the browser how to format the content.
- **Element:** An opening tag, the content, and the closing tag (e.g., `<p>Hello</p>`).

## Key Points and Lists from the Lecture
- Most tags come in pairs: an opening tag `<tag>` and a closing tag `</tag>` (which has a forward slash).
- The `<title>` tag goes inside the `<head>` and changes the text on the browser tab.
- All visible content (text, images) must be written strictly inside the `<body>` tags.
