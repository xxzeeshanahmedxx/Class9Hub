...
- **Selector:** The part of the CSS rule that identifies the HTML element to be styled (e.g., `h1` or `p`).
- **Declaration:** The property-value pair that dictates the style (e.g., `color: blue;`).

## Key Points and Lists from the Lecture
- CSS removes the need to write repetitive styling code inside every single HTML tag.
- **Syntax Example:** `h1 { color: blue; text-align: center; }`
- The curly braces `{}` are mandatory to group the declarations together.
