...
- **Hex Code:** A 6-digit code starting with `#` used to define specific colors in CSS (e.g., `#FF0000` for red).

## Key Points and Lists from the Lecture
- Colors can be written as names (e.g., `red`), Hex codes (e.g., `#FF0000`), or RGB values (e.g., `rgb(255, 0, 0)`).
- Provide backup fonts in `font-family` just in case the user doesn't have the first font installed (e.g., `font-family: Arial, sans-serif;`).
- `text-align: center;` is used to center text horizontally.
